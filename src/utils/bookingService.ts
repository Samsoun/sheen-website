...
/**
 * Storniert eine Buchung (setzt den Status auf "cancelled")
 */
export async function cancelBooking(
  bookingId: string
): Promise<{ success: boolean; error?: string }> {
  try {
    console.log(`Storniere Buchung mit ID: ${bookingId}`);

    // Referenz auf das Buchungsdokument
    const bookingRef = doc(db, "bookings", bookingId);

    // Prüfe zuerst, ob die Buchung existiert
    ...
    console.log(`Aktuelle Buchungsdaten:`, bookingData);

    // Prüfe, ob die Buchung bereits storniert ist
    if (bookingData.status === "cancelled") {
      console.warn(`Buchung ${bookingId} ist bereits storniert`);
      return {
        success: false,
    ...
    console.log(`✅ Buchung ${bookingId} erfolgreich storniert`);
    return { success: true };
  } catch (error) {
    console.error(`❌ Fehler beim Stornieren der Buchung ${bookingId}:`, error);
    return {
      success: false,
      error:
        error instanceof Error
          ? error.message
          : "Unbekannter Fehler beim Stornieren",
    };
  }
}
