...
function initStickyNavigation() {
  console.log('Initialisiere Sticky Navigation...');

  // Direkter Ansatz mit Scroll-Event für bessere Kompatibilität
  const header = document.querySelector('.header');
  const featuredSection = document.querySelector('.section-featured');

  if (!header || !featuredSection) {
    console.error('Header oder Featured-Sektion nicht gefunden!');
    return;
  }

  // Position der Featured-Sektion erfassen
  const getFeaturedPosition = () => {
    return featuredSection.getBoundingClientRect().top;
  };

  // Sticky-Klasse hinzufügen/entfernen basierend auf Scroll-Position
  const handleScroll = () => {
    const scrollPos = window.scrollY || window.pageYOffset;
    const featuredPos = getFeaturedPosition() + window.scrollY;

    // Debug-Ausgabe zur Fehlersuche
    console.log(`Scroll: ${scrollPos}, Featured Position: ${featuredPos}`);

    if (scrollPos >= featuredPos) {
      console.log('Navigation sollte sticky werden');
      document.body.classList.add('sticky');
    } else {
      console.log('Navigation sollte nicht sticky sein');
      document.body.classList.remove('sticky');
    }
  };

  // Scroll-Event-Handler registrieren
  window.addEventListener('scroll', handleScroll);

  // Initial auslösen
  handleScroll();
  console.log('Sticky Navigation initialisiert');
}

// Warte auf DOM-Ladung und führe dann die Initialisierung aus
...
// Auch nach dem Laden und Rendern des Fensters versuchen zu initialisieren
window.addEventListener('load', initStickyNavigation);
...
export function initializeApp() {
  // Hier den Inhalt Ihrer script.js einfügen, angepasst für React

  // Beispiel:
  document.addEventListener('DOMContentLoaded', function () {
    // Ihre ursprüngliche Initialisierungslogik
  });
}
