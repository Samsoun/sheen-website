...
/**
 * Erstellt eine WhatsApp-Nachricht für die kostenlose WhatsApp Business App
 */
function createWhatsAppNotification(bookingData: BookingData): void {
  ...
  const treatments = Array.isArray(bookingData.treatments)
    ? bookingData.treatments.map((t) => t.name).join(", ")
    : bookingData.service || "Unbekannte Behandlung";

  const whatsappMessage = `🎉 NEUE BUCHUNG - Sheen Beauty Studio
...
📅 Datum: ${formatDate(bookingData.date)}
⏰ Zeit: ${bookingData.time}
💰 Preis: ${bookingData.price}€

${bookingData.message ? `💬 Nachricht: ${bookingData.message}` : ""}

📍 Sheen Beauty Studio
Kurfürstendamm 63, 10717 Berlin`;

  // Erstelle einen WhatsApp-Link für die kostenlose App
  const phoneNumber = "49176328126020"; // WICHTIG: Ändere diese zu deiner WhatsApp Business Nummer (ohne +)
  const encodedMessage = encodeURIComponent(whatsappMessage);
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;

  console.log("📱 WhatsApp-Benachrichtigung erstellt:");
  console.log("Nachricht:", whatsappMessage);
  console.log("WhatsApp-Link:", whatsappUrl);

  // Öffne automatisch WhatsApp (funktioniert auf mobilen Geräten)
  if (typeof window !== "undefined") {
    try {
      // Versuche, WhatsApp automatisch zu öffnen
      window.open(whatsappUrl, "_blank");
    } catch (error) {
      console.log("WhatsApp konnte nicht automatisch geöffnet werden:", error);
    }
  }

  // Zusätzlich: Speichere die Nachricht in der Console für manuelle Nutzung
  console.log("🔗 Kopiere diesen Link und öffne ihn auf deinem Handy:");
...
export async function sendBookingConfirmationEmails(
  ...
  try {
    ...
    // 📱 WhatsApp-Benachrichtigung erstellen (kostenlose Lösung)
    try {
      createWhatsAppNotification(bookingData);
      console.log("📱 WhatsApp-Benachrichtigung erstellt ✅");
    } catch (whatsappError) {
      console.error("❌ Fehler bei WhatsApp-Benachrichtigung:", whatsappError);
    }
