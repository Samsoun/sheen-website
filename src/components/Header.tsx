...
const Header = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          ...
          if (customerData) {
            setUserName(`${customerData.firstName} ${customerData.lastName}`);
            setUserAvatar(customerData.profileImageUrl || null);
          } else {
            // Fallback auf Firebase Auth-Daten
            setUserName(user.displayName || "Kunde");
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Kommentar: Dropdown beim Scrollen nicht mehr schließen, damit es in allen Zuständen funktioniert
      // if (showDropdown) {
      //   setShowDropdown(false);
      // }
    };
  ...
  return (
    <header
      ref={headerRef}
      className={`header ${mobileNavOpen ? "nav-open" : ""}`}
    >
      ...
      <nav className="main-nav">
        <ul className="main-nav-list">
          <li>
            <Link href="/#app" className="main-nav-link">
              Sheen App
            </Link>
          </li>
          <li>
            <Link href="/#pictures" className="main-nav-link">
              Pictures
            </Link>
          </li>
          <li>
            <Link href="/#service" className="main-nav-link">
              Services
            </Link>
          </li>
          <li>
            <Link href="/#testimonials" className="main-nav-link">
              Testimonials
            </Link>
          </li>
          ...
          <li>
            {isLoggedIn ? (
              ...
              <Link
                ...
                onClick={(e) => {
                  // Navigation erfolgt automatisch durch Link
                }}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
